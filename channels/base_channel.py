...
class ProximityChat(BaseChannel):
    ...
    def send(self, sender: "BaseArcaneAgent", recipient: "BaseArcaneAgent",
             content: str, **kwargs) -> Message:
        msg = Message(
            sender_id=sender.unique_id if hasattr(sender, 'unique_id') else str(sender),
            recipient_id=recipient.unique_id if hasattr(recipient, 'unique_id') else str(recipient),
    ...
    def get_inbox(self, agent: "BaseArcaneAgent") -> list[Message]:
        aid = agent.unique_id if hasattr(agent, 'unique_id') else str(agent)
